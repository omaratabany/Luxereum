...
import "./LuxuryWatchCertificate.sol";

contract LuxuryWatchCertificateFactory {
    // Add a mapping to store contract addresses for each owner
    mapping(address => address[]) public contractsByOwner;

    event CertificateCreated(address contractAddress);
    ...
    function createCertificate(
        ...
        LuxuryWatchCertificate newCertificate = new LuxuryWatchCertificate(
            _prodID,
            _prodName,
            _manu,
            _prodDate,
        ...
        // Add the newly created contract address to the mapping
        contractsByOwner[msg.sender].push(address(newCertificate));
        emit CertificateCreated(address(newCertificate));
    }

    // Add a function to get contract addresses for a specific owner
    function getContractsByOwner(address owner)
        public
        view
